...
def get_db_connection():
    ...
    if "gcp_service_account" not in st.secrets:
        st.error("⚠️ ERRO: Você não configurou os 'Secrets' no painel do Streamlit. O app não vai salvar nada.")
        st.stop()
    ...
    # Tenta abrir a planilha pelo nome exato que está no seu print
    try:
        return client.open("enamed_db_v4.csv").sheet1
    except:
        try:
            # Tenta sem o .csv caso você renomeie
            return client.open("enamed_db_v4").sheet1
        except Exception as e:
            st.error(f"❌ ERRO DE CONEXÃO: O robô não encontrou a planilha 'enamed_db_v4.csv'. Verifique se você compartilhou ela com o email: {creds_dict.get('client_email')}")
            st.stop()

# --- FUNÇÕES DE CARREGAMENTO E SALVAMENTO ---

def init_db_online(sheet):
    # Cria o cronograma inicial
    ...
    # Força a escrita no Sheets
    try:
        sheet.clear()
        # Prepara dados (Lista de Listas)
...
def load_data():
    try:
        ...
        # Se a planilha estiver vazia (como no seu print), PREENCHE ELA AGORA
        if not data:
            with st.spinner("Inicializando Banco de Dados na Planilha... Aguarde..."):
